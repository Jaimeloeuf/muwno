...
const paymentDetails = stripeStore.getPaymentDetails();
...
onMounted(async function mountStripePaymentForm() {
  ...
  elements.value
    .create("payment", {
      defaultValues: {
        billingDetails: { email: paymentDetails.orgEmail },
      },
      wallets: {
        applePay: "auto",
        googlePay: "auto",
...
async function pay() {
  ...
  const { error } = await stripe.confirmPayment({
    ...
    confirmParams: {
      // @todo Change to payment completion page
      return_url: getAbsoluteUrlFromRoute(ManageSubscriptionRoute.name),

      // Provide a email for receipts. Stripe will send an email receipt when
      // the payment succeeds in live mode, but no in test mode.
      receipt_email: paymentDetails.orgEmail,
    },
  });
